...
function toDefaultAccountConfig(defaultChatAgent: IDefaultChatAgent | undefined): IDefaultAccountConfig {
	// Return safe defaults if no chat agent is configured
	if (!defaultChatAgent) {
		return {
			preferredExtensions: [],
...
class DefaultAccountProvider extends Disposable implements IDefaultAccountProvider {
	...
	private async fetchDefaultAccount(useExistingEntitlements: boolean): Promise<IDefaultAccountData | null> {
		...
		if (!declaredProvider) {
			this.logService.debug(`[DefaultAccount] Authentication provider is not declared.`, defaultAccountProvider);
			return null;
		}
